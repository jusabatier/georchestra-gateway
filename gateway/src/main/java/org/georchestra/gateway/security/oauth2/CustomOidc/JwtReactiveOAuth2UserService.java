...
public class JwtReactiveOAuth2UserService implements ReactiveOAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public Mono<OAuth2User> loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        return Mono.defer(() -> {
            ...
            String appContentType = MediaType.APPLICATION_JSON_VALUE;
            WebClient.RequestHeadersSpec<?> requestHeadersSpec = getRequestHeaderSpec(userRequest, userInfoUri,
                    authenticationMethod, appContentType);
            // @formatter:off
            ResponseSpec req = requestHeadersSpec.retrieve();

            Mono<Map<String, Object>> userAttributes = req
                    .onStatus(HttpStatus::isError, (response) -> parse(response).map((userInfoErrorResponse) -> {
                        ...
                        OAuth2Error oauth2Error = new OAuth2Error(INVALID_USER_INFO_RESPONSE_ERROR_CODE, description,
                                null);
                        throw new OAuth2AuthenticationException(oauth2Error, oauth2Error.toString());
                    })).bodyToMono(JwtReactiveOAuth2UserService.STRING_OBJECT_MAP);
    ...
    private WebClient.RequestHeadersSpec<?> getRequestHeaderSpec(OAuth2UserRequest userRequest, String userInfoUri,
            AuthenticationMethod authenticationMethod, String contentType) {
