...
/**
 * Configuration properties for controlling which HTTP request information is
 * included in the MDC.
 * <p>
 * These properties determine what request-related information is added to the
 * MDC (Mapped Diagnostic Context) during request processing. Including this
 * information in the MDC makes it available to all logging statements,
 * providing valuable context for debugging, monitoring, and audit purposes.
 * <p>
 * The properties are configured using the prefix
 * {@code logging.mdc.include.http} in the application properties or YAML files.
 * <p>
 ...
 * </pre>
 * <p>
 * This class provides methods to extract and add HTTP request properties to the
 * MDC based on the configuration. It supports both Servlet and WebFlux
 * environments through its flexible API.
 */
@Data
...
public class HttpRequestMdcConfigProperties {
    ...
    /**
     * Whether to append the http.request.id MDC property. The value is the id
     * provided by the http.request.id header, or a new monotonically increating UID
     * if no such header is present.
     */
    private boolean id = true;

    /**
     * Whether to append the http.request.remote-addr MDC property, interpreted as
     * the Internet Protocol (IP) address of the client or last proxy that sent the
    ...
    /**
     * Whether to append the http.request.remote-host MDC property, interpreted as
     * the fully qualified name of the client or the last proxy that sent the
     ...
     * REMOTE_HOST. Defaults to false to avoid the possible overhead in reverse DNS
    ...
    /**
     * Whether to append the http.request.query-string MDC property from the HTTP
     * request query string
     */
    private boolean queryString = false;

    /**
     * Whether to append the http.request.session.is MDC property if there's an
     * HttpSession associated to the request
     */
    private boolean sessionId = false;

    /**
     * Whether to append one http.request.cookie.[name] MDC property from each
     * request cookie
     */
    private boolean cookies = false;

    /**
     * Whether to append one http.request.header.[name] MDC property from each HTTP
     * request header whose name matches the headers-pattern
     */
    private boolean headers = false;

    /**
     * Java regular expression indicating which request header names to include when
     * logging.mdc.include.http.headers=true. Defaults to include all headers with
     * the pattern '.*'
     */
     ...
     * <p>
     * This method extracts headers from the supplied HttpHeaders and adds them to
     * the MDC if {@link #isHeaders()} is true. Only headers matching the
     * {@link #getHeadersPattern()} will be included.
     *
     ...
     * <p>
     * This method extracts cookies from the supplied MultiValueMap and adds them to
     * the MDC if {@link #isCookies()} is true. Each cookie is added with the key
     * format {@code http.request.cookie.[name]}.
     *
     ...
     * <p>
     * This method processes a list of cookies and adds them to the MDC with the key
     * format {@code http.request.cookie.[name]}. If multiple cookies with the same
     * name exist, their values are concatenated with semicolons.
     *
    ...
    /**
     * Determines if a header should be included in the MDC based on the header
     * pattern.
     * <p>
     * This method checks if the header name matches the pattern defined in
     * {@link #getHeadersPattern()}. The "cookie" header is always excluded because
     * cookies are handled separately by the {@link #cookies(Supplier)} method.
     *
     ...
     * <p>
     * This method adds a header to the MDC with the key format
     * {@code http.request.header.[name]} if the header name matches the inclusion
     * pattern. Multiple values for the same header are joined with commas.
     *
    ...
    /**
     * @return the id provided by the {@code traceId} header,
     *         {@code http.request.id} header, or a new monotonically increating UID
     *         if no such header is present
     */
    ...
    /**
     * Obtains the request id, if present, fromt the {@code trace-id},
     * {@code http.request.id}, or {@code x-request-id} request headers.
     */
    public static Optional<String> findRequestId(Supplier<HttpHeaders> headers) {
        HttpHeaders httpHeaders = headers.get();
        return header("trace-id", httpHeaders).or(() -> header(REQUEST_ID_HEADER, httpHeaders))
